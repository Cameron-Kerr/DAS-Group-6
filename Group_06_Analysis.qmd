...
ggplot(genre_probs, aes(x = genre, y = mean_prob, fill = genre)) +
  ...
  labs(title = "Average Probability of Rating > 7 for Different Movie Genres",
       x = "Movie Genre",
...
The ROC curve demonstrates that model 3 has excellent classification performance, with an AUC of 0.955, indicating strong discriminative ability. The curve is close to the top-left corner, suggesting high sensitivity and specificity, meaning the model effectively identifies positive cases while minimizing false positives.
...
***Lasso: same for model 3
```{r}
# lasso: also remove votes, Animation and Romance
library(glmnet)
