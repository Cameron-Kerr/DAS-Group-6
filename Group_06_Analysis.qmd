...
The summary statistics help better understand the data. It is worth noting there are massive outliers in the data for the Length, Budget and Votes variables which may be influential on our modeling. This means that the median statistic is going to be more informative when reading this table as it will not be influenced by outliers.
...
The main genre's within our dataset are Drama, Action and Comedy.The  Animation, Documentary and Short genre also compose around 15% of the of the data. There are only 18 Romance movies in the data, composing only 1% of the proportion of the data.
...
The data had to be log-transformed so the boxplots could be interpretable, (IS THIS FAN VOTES?)
...
This table above helps us interpret the boxplots of the genres above instead this time in a numerical format, it also gives us the count data for each genre. (SHOULD I ADD MORE?)
